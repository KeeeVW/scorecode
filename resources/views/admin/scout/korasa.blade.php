...
@section('title', 'Scout Korasa Management')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Scout Korasa Records</h5>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createKorasaModal">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach($users as $user)
                                    @foreach($user->korasaRecords as $record)
                                    <tr>
                                        <td>{{ $user->name }}</td>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-sm btn-info" 
                                                        ...
                                                        data-bs-target="#editKorasaModal{{ $record->id }}">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <form action="{{ route('admin.scout.korasa.delete', $record->id) }}" 
                                                      method="POST" 
                                                      class="d-inline"
...
<!-- Create Korasa Record Modal -->
<div class="modal fade" id="createKorasaModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('admin.scout.korasa.store') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Korasa Record</h5>
...
<!-- Edit Korasa Record Modals -->
@foreach($users as $user)
    @foreach($user->korasaRecords as $record)
    <div class="modal fade" id="editKorasaModal{{ $record->id }}" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.scout.korasa.update', $record->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Korasa Record</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
